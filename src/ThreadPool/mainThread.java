...
public class mainThread implements Runnable {
  ...
  public mainThread(int driverid, String linkid, t_neo4j_node_info_node startinfonode,
  ...
  public void run() {
    ...
    try {
      ...
      judges = t_neo4j_link_judge_dao.getallnode(linkid);// 获取到该关系线的过滤条件
      startnodepro = t_neo4j_node_property_dao.getnodepropertybyid(startid);// 起始点属性
      endnodepro = t_neo4j_node_property_dao.getnodepropertybyid(endid);// 结束属性
      linkpro = t_neo4j_link_property_dao.getlinkpropertybyid(linkid);// 关系属性

      getjudates();// 进行获取原始表数据，全量与增量
    } catch (Exception e) {
      e.printStackTrace();
      logger.error(e);
    } finally {
  ...
  /**
   * 根据过滤条件进行数据库查询，分页批量获取起始表数据
   * 
   * @return
   */
  private void getjudates() throws Exception {
    // 获取到开始表的总数量，分批查询
    java.util.Map map = null;
    String endzlval = sysdao.getstarttablecount(zlname, zltype, startusername, starttablename);// 获取到最大增量值

    Thread.sleep(1500);

    if ("".equals(zlval) || "0".equals(zlval) || zlval == null) {// 如果增量字段为空或者是0，全量抽取
      map =
          (java.util.Map) sysdao.getstarttablecount(startusername, starttablename, zlname, zltype,
              "", endzlval).get(0);
    } else {
      map =
          (java.util.Map) sysdao.getstarttablecount(startusername, starttablename, zlname, zltype,
              zlval, endzlval).get(0);

    }
    int startco = 0;
    if (map.get("CN") != null) {
      String cosss = (String) map.get("CN");
      startco = Integer.parseInt(cosss);
    }
    int fenco = startco / util.pagesize;
    int yuco = startco % util.pagesize;
    if (yuco > 0) {// 如果余数大于0那么分页的页码要加1
      fenco = fenco + 1;
    }
    ArrayList startdates = new ArrayList();
    for (int i = 1; i <= fenco; i++) {
      ...
      if ("".equals(zlval) || "0".equals(zlval) || zlval == null) {// 如果增量字段为空或者是0，全量抽取
        startdates =
            sysdao.getjudgedates(i, util.pagesize, startusername, starttablename, judges,
                startnodepro, zlname, zltype, "", endzlval);
        // 全量抽取的时候，再开一次线程池
        Thread thread =
            new Thread(new allsearchMainThread(linkid, startinfonode, endinfonode, linkname,
                aspect_type, startdates, judges, startnodepro, endnodepro, linkpro));// 批量添加结点数据的线程
        ReadController.executorsearchServices.submit(thread);

      } else {// 增量抽取
        startdates =
            sysdao.getjudgedates(i, util.pagesize, startusername, starttablename, judges,
                startnodepro, zlname, zltype, zlval, endzlval);
        biduistatoend(startdates, judges);
      }


    }
    // if (fenco != 0) {// 分页数不能为0,应该是为空也要保存
    sysdao.savezlval(zltype, endzlval, startid);// 保存最大增量值
    logger.error(startid + "此次增量值为" + endzlval);
    // }
